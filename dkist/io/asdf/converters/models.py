...
class CoupledCompoundConverter(TransformConverterBase):
    ...
    def from_yaml_tree_transform(self, node, tag, ctx):
        ...
        if not isinstance(left, Model):
            raise TypeError("Unknown model type '{model}'".format(model=node["forward"][0]._tag))  # pragma: no cover

        right = node["forward"][1]
        if (not isinstance(right, Model) and
                not (oper == "fix_inputs" and isinstance(right, dict))):
            raise TypeError("Unknown model type '{model}'".format(model=node["forward"][1]._tag))  # pragma: no cover
